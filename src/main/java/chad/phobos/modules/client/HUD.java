...
public class HUD extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = Client.FULLMODEVER;
            ...
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < Client.moduleManager.sortedModulesABC.size(); k++) {
                        String str = Client.moduleManager.sortedModulesABC.get(k);
        ...
        String direction = this.direction.getValue().booleanValue() ? Client.rotationManager.getDirection4D(false) : "";
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (Client.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) Client.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
