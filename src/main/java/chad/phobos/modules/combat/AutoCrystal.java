package chad.phobos.modules.combat;

import chad.phobos.api.utils.*;
import chad.phobos.api.utils.Timer;
import chad.phobos.api.center.Module;
import chad.phobos.modules.client.ClickGui;
import chad.phobos.modules.misc.AutoGG;
...
public class AutoCrystal
    ...
    public AutoCrystal() {
        super("AutoCrystal", "best ca 0n m4rk37", Module.Category.COMBAT, true, false, false);
    }
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            if (AutoGG.getINSTANCE().isOn()) {
                AutoGG autoGG = (AutoGG) Client.moduleManager.getModuleByName("AutoGG");
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            if (AutoCrystal.mc.player == null || AutoCrystal.mc.player.isDead || entity.isDead || entity == AutoCrystal.mc.player || Client.friendManager.isFriend(entity.getName()) || entity.getDistance(AutoCrystal.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
