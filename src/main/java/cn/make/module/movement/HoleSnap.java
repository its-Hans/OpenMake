...
extends Module {
    ...
    private final Setting<Integer> range = rinte("Range", 5, 1, 50);
    private final Setting<Float> timer = rfloa("Timer", 1.6f, 1.0f, 8.0f);
    private final Setting<Integer> timeoutTicks = rinte("TimeOutTicks", 10, 0, 30);
    ...
    private boolean shouldDisable(double currentSpeed) {
        ...
        if (blockPos != null) {
            ...
            if (HoleSnap.is2HoleB(blockPos) && this.toBlockPos(this.toVec3dCenter(HoleSnap.mc.player.getPosition())).equals(blockPos)) {
                return true;
            }
        }
    ...
    private BlockPos getHole() {
        if (HoleSnap.mc.player.ticksExisted % 10 == 0 && !this.getFlooredPosition(HoleSnap.mc.player).equals(this.holePos)) {
    ...
    private BlockPos findHole() {
        ...
        for (BlockPos blockPos : posList) {
            BlockPos pos;
            EntityPlayerSP entityPlayerSP2 = HoleSnap.mc.player;
            double dist = this.distanceTo(entityPlayerSP2, blockPos);
            if (!(dist <= (double) this.range.getValue()) || dist > closestHole.getLeft())
                continue;
            int n2 = 0;
            while (n2 < 6 && HoleSnap.mc.world.isAirBlock((pos = blockPos.add(0, -n2++, 0)).up())) {
