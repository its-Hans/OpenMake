package cn.make.newgui.components;

import cn.make.newgui.PhobosGui;
import cn.make.newgui.components.items.Item;
import cn.make.newgui.components.items.buttons.Button;
import chad.phobos.Client;
import chad.phobos.api.center.Feature;
import chad.phobos.modules.client.ClickGui;
import chad.phobos.api.utils.ColorUtil;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.open) {
            ...
            if (ClickGui.getInstance().outline.getValue()) {
                    ...
                    GL11.glColor4f(
                        (float)(
                            (float) ClickGui.getInstance().oRed.getValue().intValue() / 255.0f
                        ),
                        (float)(
                            (float) ClickGui.getInstance().oGreen.getValue().intValue() / 255.0f
                        ),
                        (float)(
                            (float) ClickGui.getInstance().oBlue.getValue().intValue() / 255.0f
                        ),
                        (float)(
                            (float) ClickGui.getInstance().oAlpha.getValue().intValue() / 255.0f
                        )
                    );
        ...
        Client.textManager.drawStringWithShadow(this.getName(), (float) this.x + 3.0f, (float) this.y - 4.0f - (float) PhobosGui.getClickGui().getTextOffset(), -1);
        if (this.open) {
