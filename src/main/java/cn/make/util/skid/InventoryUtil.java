...
implements Util {
    ...
    public static int getItemHotbars(Item item) {
        for (int i = 0; i < 36; ++i) {
            Item item2 = mc.player.inventory.getStackInSlot(i).getItem();
    ...
    public static void switchToHotbarSlot(int n, boolean bl) {
        if (mc.player.inventory.currentItem == n || n < 0) {
            return;
        }
        if (bl) {
    ...
    private static Map<Integer, ItemStack> getInventorySlots() {
        ...
        for (int i = 9; i <= 44; ++i) {
            hashMap.put(i, mc.player.inventoryContainer.getInventory().get(i));
        }
        return hashMap;
    }
    ...
    public static int pickItem(int n) {
        ...
        for (int i = 0; i < 9; ++i) {
            if (Item.getIdFromItem(mc.player.inventory.mainInventory.get(i).getItem()) != n) continue;
    ...
    public static boolean isSlotEmpty(int n) {
        Slot slot = mc.player.inventoryContainer.inventorySlots.get(n);
        ItemStack itemStack = slot.getStack();
