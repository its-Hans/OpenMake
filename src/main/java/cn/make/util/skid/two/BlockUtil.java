...
public class BlockUtil implements Util
{
    ...
    public static ValidResult valid(final BlockPos pos) {
        ...
        if (l_State.getBlock() == Blocks.AIR) {
            ...
            final BlockPos[] array4;
            final BlockPos[] array2 = array4 = (array3 = (l_Blocks = new BlockPos[] { pos.north(), pos.south(), pos.east(), pos.west(), pos.up(), pos.down() }));
            for (final BlockPos l_Pos : array4) {
    ...
    public static void noSwingPlace(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet) {
        final EnumFacing side = getFirstFacing(pos);
        if (side == null) {
            return;
        }
        ...
        final Block neighbourBlock = BlockUtil.mc.world.getBlockState(neighbour).getBlock();
        if (
            !BlockUtil.mc.player.isSneaking()
                && (BlockUtil.blackList.contains(neighbourBlock)
                || BlockUtil.shulkerList.contains(neighbourBlock)
            )
        ) {
            BlockUtil.mc.player.connection.sendPacket(
    ...
    public static void burrowPlaceSync(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet) {
        final EnumFacing side = getFirstFacing(pos);
        if (side == null) {
            return;
        }
