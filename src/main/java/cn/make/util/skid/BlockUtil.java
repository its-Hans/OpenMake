...
implements Util {
    ...
    static Entity checkEntity(Vec3d[] vec3dArray, BlockPos blockPos) {
        ...
        for (Vec3d vec3d : vec3dArray) {
            ...
            for (Entity entity2 : mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos2))) {
    ...
    public static boolean PowerCheck(BlockPos blockPos) {
        ...
        if (mc.world.getBlockState(blockPos).getBlock() != Blocks.AIR && mc.world.getBlockState(blockPos).getBlock() != Blocks.REDSTONE_BLOCK) {
            return true;
        }
    ...
    public static void placeCrystalOnBlock(BlockPos blockPos, EnumHand enumHand, boolean bl, boolean bl2, boolean bl3) {
        RayTraceResult rayTraceResult = mc.world.rayTraceBlocks(new Vec3d(mc.player.posX, mc.player.posY + (double) mc.player.getEyeHeight(), mc.player.posZ), new Vec3d((double)blockPos.getX() + 0.5, (double)blockPos.getY() - 0.5, (double)blockPos.getZ() + 0.5));
        ...
        if (enumHand == EnumHand.MAIN_HAND && bl3 && n2 != -1 && n2 != mc.player.inventory.currentItem) {
            mc.player.connection.sendPacket(new CPacketHeldItemChange(n2));
        }
        mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(blockPos, enumFacing, enumHand, 0.0f, 0.0f, 0.0f));
        if (enumHand == EnumHand.MAIN_HAND && bl3 && n2 != -1 && n2 != mc.player.inventory.currentItem) {
            mc.player.connection.sendPacket(new CPacketHeldItemChange(n));
        }
        if (bl) {
            mc.player.connection.sendPacket(new CPacketAnimation(bl2 ? enumHand : EnumHand.MAIN_HAND));
        }
    }
    ...
    public static void placeBlock(BlockPos blockPos, EnumHand enumHand, boolean bl, boolean bl2) throws Exception {
        ...
        if (!mc.player.isSneaking() && (blackList.contains(block) || shulkerList.contains(block))) {
    ...
    public static EnumFacing getRayTraceFacing(BlockPos blockPos) {
        RayTraceResult rayTraceResult = mc.world.rayTraceBlocks(new Vec3d(mc.player.posX, mc.player.posY + (double) mc.player.getEyeHeight(), mc.player.posZ), new Vec3d((double)blockPos.getX() + 0.5, (double)blockPos.getX() - 0.5, (double)blockPos.getX() + 0.5));
    ...
    public static void rotatePacket(double d, double d2, double d3) {
        double d4 = d - mc.player.posX;
        double d5 = d2 - (mc.player.posY + (double) mc.player.getEyeHeight());
        double d6 = d3 - mc.player.posZ;
    ...
    public static void sneak(BlockPos blockPos) {
        if (!mc.player.isSneaking() && (blackList.contains(blockPos) || shulkerList.contains(blockPos))) {
            mc.player.connection.sendPacket(new CPacketEntityAction(mc.player, CPacketEntityAction.Action.START_SNEAKING));
            mc.player.setSneaking(true);
        }
    }
    ...
    public static NonNullList possiblePlacePositions(float f, boolean bl, boolean bl2) {
        ...
        nonNullList.addAll(BlockUtil.getSphere(EntityUtil.getPlayerPos(mc.player), f, (int)f, false, true, 0).stream().filter(arg_0 -> BlockUtil.possiblePlacePositions1(bl, bl2, arg_0)).collect(Collectors.toList()));
        return nonNullList;
    }

    public static NonNullList getBlockSphere(float f, Class clazz) {
        NonNullList nonNullList = NonNullList.create();
        nonNullList.addAll(BlockUtil.getSphere(EntityUtil.getPlayerPos(mc.player), f, (int)f, false, true, 0).stream().filter(arg_0 -> BlockUtil.getBlockSphere0(clazz, arg_0)).collect(Collectors.toList()));
        return nonNullList;
    }

    public static boolean isAir(BlockPos blockPos) {
        return mc.world.getBlockState(blockPos).getBlock().equals(Blocks.AIR);
    }

    public static double distanceToXZ(double d, double d2) {
        double d3 = mc.player.posX - d;
        double d4 = mc.player.posZ - d2;
        return Math.sqrt(d3 * d3 + d4 * d4);
    }

    public static boolean isScaffoldPos(BlockPos blockPos) {
        return mc.world.isAirBlock(blockPos) || mc.world.getBlockState(blockPos).getBlock() == Blocks.SNOW_LAYER || mc.world.getBlockState(blockPos).getBlock() == Blocks.TALLGRASS || mc.world.getBlockState(blockPos).getBlock() instanceof BlockLiquid;
    }
    ...
    public static boolean isAir(double d, double d2, double d3) {
        ...
        return mc.world.getBlockState(BlockUtil.vec3toBlockPos(vec3d, true)).getBlock().equals(Blocks.AIR);
    }
    ...
    public static boolean CanPlace(BlockPos blockPos) {
        for (EnumFacing enumFacing : EnumFacing.VALUES) {
            if (!BlockUtil.isReplaceable(blockPos) || noPlaceBlock.contains(BlockUtil.getBlock(blockPos.offset(enumFacing))) || !(mc.player.getDistanceSq(blockPos) <= MathUtil.square(5.0))) continue;
            return true;
        }
        return false;
    }
    ...
    public static void attackCrystals(BlockPos blockPos, boolean bl) {
        ...
        int n = Client.serverManager.getPing();
        for (EntityEnderCrystal entityEnderCrystal : mc.world.getEntitiesWithinAABB(EntityEnderCrystal.class, new AxisAlignedBB(blockPos))) {
            long l = n <= 50 ? 75L : 100L;
            if (!attackTimer.passedMs(l)) continue;
            if (bl) {
                Client.newRotationManager.lookAtVec3dPacket(entityEnderCrystal.getPositionVector(), false, true);
            }
            if (bl2) {
    ...
    public static boolean isBlockUnSolid(BlockPos blockPos) {
        return isBlockUnSolid(mc.world.getBlockState(blockPos).getBlock());
    }
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos, boolean bl, boolean bl2) {
        ...
        try {
            ...
            if (!bl2 && mc.world.getBlockState(blockPos3).getBlock() != Blocks.AIR || mc.world.getBlockState(blockPos2).getBlock() != Blocks.AIR) {
                return false;
            }
            for (Entity entity : mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos2))) {
                if (entity.isDead) continue;
                if (!bl || !(entity instanceof EntityEnderCrystal)) return false;
            }
            if (bl2) return true;
            for (Entity entity : mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos3))) {
                if (entity.isDead) continue;
                if (!bl || !(entity instanceof EntityEnderCrystal)) return false;
            }
            return true;
    ...
    public static double getMineDistance(BlockPos blockPos) {
        return BlockUtil.getMineDistance(mc.player, blockPos);
    }
    ...
    public static ArrayList<BlockPos> haveNeighborBlock(BlockPos blockPos, Block block) {
        ...
        if (mc.world.getBlockState(blockPos.add(1, 0, 0)).getBlock().equals(block)) {
            arrayList.add(blockPos.add(1, 0, 0));
        }
        if (mc.world.getBlockState(blockPos.add(-1, 0, 0)).getBlock().equals(block)) {
            arrayList.add(blockPos.add(-1, 0, 0));
        }
        if (mc.world.getBlockState(blockPos.add(0, 1, 0)).getBlock().equals(block)) {
            arrayList.add(blockPos.add(0, 1, 0));
        }
        if (mc.world.getBlockState(blockPos.add(0, -1, 0)).getBlock().equals(block)) {
            arrayList.add(blockPos.add(0, -1, 0));
        }
        if (mc.world.getBlockState(blockPos.add(0, 0, 1)).getBlock().equals(block)) {
    ...
    public static void placeBlock(BlockPos blockPos, boolean bl, boolean bl2, boolean bl3, boolean bl4) {
        if (BlockUtil.canReplace(blockPos)) {
            ...
            if ((optional = BlockUtil.getClickLocation(blockPos, bl4, false, bl3)).isPresent()) {
                ...
                if (bl) {
                    Client.newRotationManager.lookAtVec3dPacket(vec3d, false, true);
                }
                if (bl2) {
                    ...
                    mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(blockPos2, enumFacing, EnumHand.MAIN_HAND, f, f2, f3));
                } else {
                    mc.playerController.processRightClickBlock(mc.player, mc.world, blockPos2, enumFacing, vec3d, EnumHand.MAIN_HAND);
                }
                mc.player.connection.sendPacket(new CPacketAnimation(EnumHand.MAIN_HAND));
                if (bl5) {
    ...
    public static Optional<ClickLocation> getClickLocation(BlockPos blockPos, boolean bl, boolean bl2, boolean bl3) {
        ...
        for (EnumFacing enumFacing2 : EnumFacing.values()) {
            ...
            if (bl2 && mc.world.getBlockState(blockPos2).getBlock() == Blocks.PISTON) {
                continue;
            }
            if (!mc.world.getBlockState(blockPos2).getBlock().canCollideCheck(mc.world.getBlockState(blockPos2), false)) {
                continue;
            }
        ...
        if (!mc.world.getBlockState(entity2).getBlock().canCollideCheck(mc.world.getBlockState(entity2), false)) {
            return Optional.empty();
        }
    ...
    public static boolean cantPistonPlace(BlockPos blockPos) {
        if (mc.world.getBlockState(blockPos.add(0, 0, 1)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(0, 0, -1)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(1, 0, 0)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(-1, 0, 0)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(0, 1, 0)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos).getBlock() != Blocks.AIR) {
            return true;
        }
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos) {
        ...
        BlockPos blockPos3 = blockPos.add(0, 2, 0);
        try {
            return (mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && mc.world.getBlockState(blockPos2).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos3).getBlock() == Blocks.AIR && mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos2)).isEmpty() && mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos3)).isEmpty();
        }
        catch (Exception exception) {
            return false;
        }
    ...
    public static boolean canBreak(BlockPos blockPos) {
        ...
        return block.getBlockHardness(iBlockState, mc.world, blockPos) != -1.0f;
    }
    public static boolean canBreakLiquidCheck(BlockPos blockPos) {
        IBlockState iBlockState = mc.world.getBlockState(blockPos);
        if (iBlockState instanceof BlockLiquid) return false;
        return iBlockState.getBlockHardness(mc.world, blockPos) > 0;
    }
    public static boolean canPlaceMake(BlockPos blockPos) {
        IBlockState state = mc.world.getBlockState(blockPos);
        if (state instanceof BlockAir) return true;
        if (state instanceof BlockLiquid) return true;
        return false;
    }
    ...
    public static int isPositionPlaceable(BlockPos blockPos, boolean bl, boolean bl2) {
        ...
        if (bl2) {
            for (Entity enumFacing : mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(blockPos))) {
    ...
    public static boolean rayTracePlaceCheck(BlockPos blockPos, boolean bl, float f) {
        return bl && mc.world.rayTraceBlocks(new Vec3d(mc.player.posX, mc.player.posY + (double) mc.player.getEyeHeight(), mc.player.posZ), new Vec3d(blockPos.getX(), (float)blockPos.getY() + f, blockPos.getZ()), false, true, false) != null;
    }
    ...
    public static Boolean isPosInFov(BlockPos blockPos) {
        ...
        if (n == 0 && (double)blockPos.getZ() - mc.player.getPositionVector().z < 0.0) {
            return false;
        }
        if (n == 1 && (double)blockPos.getX() - mc.player.getPositionVector().x > 0.0) {
            return false;
        }
        if (n == 2 && (double)blockPos.getZ() - mc.player.getPositionVector().z > 0.0) {
            return false;
        }
        return n != 3 || (double)blockPos.getX() - mc.player.getPositionVector().x >= 0.0;
    }
    ...
    public static List<EnumFacing> getPossibleSides(BlockPos blockPos) {
        ...
        for (EnumFacing enumFacing : EnumFacing.values()) {
            BlockPos blockPos2 = blockPos.offset(enumFacing);
            if (!mc.world.getBlockState(blockPos2).getBlock().canCollideCheck(mc.world.getBlockState(blockPos2), false) || mc.world.getBlockState(blockPos2).getMaterial().isReplaceable()) continue;
            arrayList.add(enumFacing);
        }
        return arrayList;
    }

    public static boolean placeBlock(BlockPos blockPos, EnumHand enumHand, boolean rotate, boolean packet, boolean bl3) {
        boolean bl4 = false;
        EnumFacing enumFacing = BlockUtil.getFirstFacing(blockPos);
        if (enumFacing == null) {
            return bl3;
        ...
        BlockUtil.rightClickBlock(blockPos2, vec3d, enumHand, enumFacing2, packet);
        mc.player.swingArm(EnumHand.MAIN_HAND);
    ...
    public static boolean isBlockEmpty(BlockPos blockPos) {
        block3: {
            try {
                if (!emptyBlocks.contains(mc.world.getBlockState(blockPos).getBlock())) break block3;
    ...
    public static boolean placeBlock(BlockPos blockPos, int n, boolean bl, boolean bl2) {
        if (BlockUtil.isBlockEmpty(blockPos)) {
            ...
            if (n != mc.player.inventory.currentItem) {
                n2 = mc.player.inventory.currentItem;
    ...
    private static boolean getBlockSphere0(Class clazz, BlockPos blockPos) {
        return clazz.isInstance(mc.world.getBlockState(blockPos).getBlock());
    }
    ...
    public static NonNullList possiblePlacePositions(float f) {
        NonNullList nonNullList = NonNullList.create();
        nonNullList.addAll(BlockUtil.getSphere(EntityUtil.getPlayerPos(mc.player), f, (int)f, false, true, 0).stream().filter(BlockUtil::canPlaceCrystal).collect(Collectors.toList()));
        return nonNullList;
    }
    ...
    public static boolean cantBlockPlace2(BlockPos blockPos) {
        if (mc.world.getBlockState(blockPos.add(0, 0, 1)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(0, 0, -1)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(1, 0, 0)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(-1, 0, 0)).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(0, 1, 0)).getBlock() == Blocks.AIR) {
            return mc.world.getBlockState(blockPos.add(0, -1, 0)).getBlock() == Blocks.AIR;
        }
        return false;
    }
